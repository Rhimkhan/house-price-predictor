...
with col1:
    ...
    washroom = st.number_input(
        "Washroom (Full Bathrooms)",
        min_value=0,
        max_value=10,
        value=2,
        step=1,
        help="Number of full bathrooms with shower/bathtub"
    )
    
    # ✅ CHANGED: Half Bathrooms → Bathroom
    bathroom = st.number_input(
        "Bathroom (Half Bathrooms)",
        min_value=0,
        max_value=5,
        value=1,
        step=1,
        help="Number of half bathrooms (toilet + sink only)"
    )
...
currency = st.sidebar.radio(
    ...
    index=0,
    help="Choose the currency for price prediction"
)
...
if predict_btn:
    with st.spinner("Calculating price..."):
        try:
            ...
            with col_result2:
                formatted_price = f"{currency_symbol} {predicted_price:,.2f}"
                ...
                <div style="text-align: center; padding: 30px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; color: white;">
                    ...
                    <h1 style="margin: 10px 0; font-size: 3.5rem;">{formatted_price}</h1>
            ...
            with st.expander("📋 Input Summary"):
                ...
                with col_a:
                    ...
                    st.write(f"- Washroom (Full): {washroom}")      # ✅ Updated
                    st.write(f"- Bathroom (Half): {bathroom}")      # ✅ Updated
            ...
            st.info("Make sure the model features match the input features.")
